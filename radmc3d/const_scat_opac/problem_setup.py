...
iformat  = 2                                  # (1) abs only; (2) + scat; (3) + anisotropy g

kappa_star_cgs = 10                           # absorption opacity [cm^2/g]
dgratio  = 100                                # dust-to-gas ratio
small_grain_ratio = 0.02184
kappa_abs  = kappa_star_cgs*dgratio*small_grain_ratio # [cm^2/g]
kappa_scat = kappa_abs
...
with open(f'dustkappa_{name}.inp', 'w+') as f:
    ...
    f.write(f'{nlam:d}\n\n')
    for value in lam:
        f.write(f'{value:13.6e} {kappa_abs:13.6e} {kappa_scat:13.6e}\n')
