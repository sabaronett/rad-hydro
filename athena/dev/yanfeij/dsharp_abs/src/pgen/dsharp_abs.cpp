...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  if (nfreq > 1) {
    ...
    // if (fkappa_sf_table == NULL) {
    //   scattering = false;
    // } else {
    //   scattering = true;
    // }
    if (fkappa_rf_table == NULL) {
      msg << "### FATAL ERROR in function [Mesh::InitUserMeshData]" << std::endl
          ...
          for (int ifr=0; ifr<nfreq; ++ifr) {
            ...
            if (ifr < nfreq-1) {
              ir(k,j,is-i,ifr*nang+nang-2) =
                  F*prad->IntPlanckFunc(prad->nu_grid(ifr)/t_star, prad->nu_grid(ifr+1)/t_star)\
                  /prad->wmu(0);                // enable outward radial ray
            } else {
              ir(k,j,is-i,ifr*nang+nang-2) =
                  F*(1.0 - prad->FitIntPlanckFunc(prad->nu_grid(ifr)/t_star))\
                  /prad->wmu(0);                // enable outward radial ray
            }
...
void DiskOpacity(MeshBlock *pmb, AthenaArray<Real> &prim)
{
  ...
  Real temp = 0.0;         // cell temperature
  ...
  for (int k=kl; k<=ku; ++k) {
    for (int j=jl; j<=ju; ++j) {
      for (int i=il; i<=iu; ++i) {
        temp = prim(IEN,k,j,i)/prim(IDN,k,j,i);
        for (int ifr=0; ifr < nfreq; ++ifr) {
          if (nfreq > 1) { GetOpacities(temp, ifr, kappa_af, kappa_pf); }
          //   if (scattering) {
