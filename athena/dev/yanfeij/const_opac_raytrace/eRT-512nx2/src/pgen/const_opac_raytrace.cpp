      ...
      for (int j=js; j<=je; ++j) {
        for (int i=is; i<=ie; ++i) {
          for (int ifr=0; ifr < nfreq; ++ifr) {                         // opacities
            ...
            pnrrad->sigma_p(k,j,i,ifr) = 0.0 * phydro->u(IDN,k,j,i)*kappa_a;  // Planck (aT^4)
          }
  ...
  for (int k=ks; k<=ke; ++k) {
    for (int j=js; j<=je; ++j) {
      for (int i=1; i<=ngh; ++i) {
        ...
        ir(k,j,is-i,prad->nang-2) = 1.0; // enable outward radial angle
    ...
    for (int j=jl; j<=ju; ++j) {
      for (int i=il; i<=iu; ++i) {
        for (int ifr=0; ifr < prad->nfreq; ++ifr) {            // opacities
          ...
          prad->sigma_p(k,j,i,ifr) = 0.0 * prim(IDN,k,j,i)*kappa_a;  // Planck mean (aT^4)
        }
