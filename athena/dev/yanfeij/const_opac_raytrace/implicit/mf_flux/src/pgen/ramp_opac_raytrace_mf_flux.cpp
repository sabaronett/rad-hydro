    ...
    for (int j=jl; j<=ju; ++j) {
      for (int i=il; i<=iu; ++i) {
        for (int ifr=0; ifr < prad->nfreq; ++ifr) {            // opacities
          ...
          if (i < 26) {
            prad->sigma_a(k,j,i,ifr) = 0.04*i*prim(IDN,k,j,i)*kappa_a;
            prad->sigma_pe(k,j,i,ifr) = 0.04*i*prim(IDN,k,j,i)*kappa_a;
            prad->sigma_p(k,j,i,ifr) = 0.04*i*prim(IDN,k,j,i)*kappa_a;
          } else {
